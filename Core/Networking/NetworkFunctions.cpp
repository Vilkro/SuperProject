...
static void BroadcastPlayerVoteTally(INDEX ctYes, INDEX ctNo) {  // 1111
    ...
    if (_bPlayerVoteIsBan) {
        ...
        strTitleRU.PrintF("^c00ff80Ãîëîñîâàíèå: çàáàíèòü ^c80ffff%s^c00ff80 íà 20 ìèíóò:^r",  _strPlayerVoteName.Undecorated().str_String);
    } else {
        strTitleEN.PrintF("^c00ff80Vote to kick ^c80ffff%s^c00ff80:^r",            _strPlayerVoteName.Undecorated().str_String);
        strTitleRU.PrintF("^c00ff80Ãîëîñîâàíèå: êèêíóòü ^c80ffff%s^c00ff80:^r",   _strPlayerVoteName.Undecorated().str_String);
    }
    BroadcastVoteMessage(strSender, strTitleEN, strTitleRU);
...
static void TallyPlayerVote() {  // 1111
    ...
    if (ctYes >= ctNeeded) {
        ...
        if (bBan) {
            ...
            strMsgRU.PrintF("^c00ff80Ãîëîñîâàíèå: ^c80ffff%s^c00ff80 çàáàíåí íà %d ìèíóò",      strName.Undecorated().str_String, PLAYERVOTE_BAN_MINUTES);
        } else {
            strMsgEN.PrintF("^c00ff80Vote passed: ^c80ffff%s^c00ff80 was kicked", strName.Undecorated().str_String);
            strMsgRU.PrintF("^c00ff80Ãîëîñîâàíèå: ^c80ffff%s^c00ff80 êèêíóò",     strName.Undecorated().str_String);
        }
        ...
        if (bBan) {
			...
			INetwork::SendDisconnectMessage(
                ...
                SelectLangMessage(iTarget,
                    ...
                    "\n^cff1111ÇÀáÀÍÅÍ ÃÎËÎÑÎÂÀÍÈÅÌ (20 ìèí)^r"),
                FALSE);
        } else {
            INetwork::SendDisconnectMessage(
                iTarget,
    ...
    if (ctYes + ctNo >= ctActive) {
        ...
        if (_bPlayerVoteIsBan) {
            ...
            strMsgRU.PrintF("^c00ff80Ãîëîñîâàíèå (çàáàíèòü ^c80ffff%s^c00ff80) íå ïðîøëî", _strPlayerVoteName.Undecorated().str_String);
        } else {
            strMsgEN.PrintF("^c00ff80Vote to kick ^c80ffff%s^c00ff80 failed",             _strPlayerVoteName.Undecorated().str_String);
            strMsgRU.PrintF("^c00ff80Ãîëîñîâàíèå (êèêíóòü ^c80ffff%s^c00ff80) íå ïðîøëî",  _strPlayerVoteName.Undecorated().str_String);
        }
        ResetPlayerVote();
        BroadcastVoteMessage("^cffff", strMsgEN, strMsgRU);
        return;
...
void ListClients(SHELL_FUNC_ARGS) {
    ...
    if (!bAny) {
        INetwork::SendChatToClient(iTo, "^ced2675", SelectLangMessage(iTo,
            ...
            "^c80ffff(íåò äðóãèõ àêòèâíûõ èãðîêîâ)^r"));
        return;
    }
}
...
void PlayerVoteTimeoutCheck(SHELL_FUNC_ARGS) {  // 1111
    ...
    if (_bPlayerVoteIsBan) {
        ...
        strMsgRU.PrintF("^c00ff80Ãîëîñîâàíèå (çàáàíèòü ^c80ffff%s^c00ff80) èñòåêëî ïî âðåìåíè",          _strPlayerVoteName.Undecorated().str_String);
    } else {
        strMsgEN.PrintF("^c00ff80Vote to kick ^c80ffff%s^c00ff80 timed out",           _strPlayerVoteName.Undecorated().str_String);
        strMsgRU.PrintF("^c00ff80Ãîëîñîâàíèå (êèêíóòü ^c80ffff%s^c00ff80) èñòåêëî ïî âðåìåíè",           _strPlayerVoteName.Undecorated().str_String);
    }
    ResetPlayerVote();
    BroadcastVoteMessage("^cffff", strMsgEN, strMsgRU);
}
